...
const NON_FICTION_FORMATS = ['Hardcover', 'Paperback', 'Kindle', 'Audible'];

export default function NonFictionBrowse() {
  // Non-fiction is identified by work_type, but we can only filter by format at the artifact level.
  // We fetch the same novel-like formats and the page title distinguishes it.
  // In a real app, we'd have a dedicated API filter for genre/work_type.
  const queries = NON_FICTION_FORMATS.map((fmt) =>
    useQuery({
  ...
  return (
    <div className="min-h-screen bg-surface px-6 py-10 lg:px-12">
      <header className="mb-10">
        <h1 className="font-headline text-5xl text-primary">Non-Fiction</h1>
        <p className="mt-2 font-body text-base text-on-surface-variant">
          {isLoading
            ? 'Loading...'
            : `${allArtifacts.length} item${allArtifacts.length !== 1 ? 's' : ''}`}
        </p>
      </header>
      ...
      {isError && (
        <div className="rounded-2xl bg-error-container p-6 text-on-error-container">
          <p className="font-body text-sm">
            Failed to load items. {firstError instanceof Error ? firstError.message : 'Unknown error.'}
          </p>
        </div>
      )}
      ...
      {!isLoading && !isError && allArtifacts.length === 0 && (
        <div className="flex flex-col items-center gap-4 py-20">
          ...
          <p className="font-body text-sm text-on-surface-variant">
            No non-fiction items in your library yet.
          </p>
        </div>
      )}
